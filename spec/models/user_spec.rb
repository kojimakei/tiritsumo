  ...
  describe 'ユーザー新規登録' do
    ...
    context '新規登録がうまくいかないとき' do
      ...
      it 'deadlineが空では登録できない' do
        @user.deadline = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("Deadlineが入力されていません。")
      end
      it 'goalが空では登録できない' do
        @user.goal = ''
        @user.valid?
        expect(@user.errors.full_messages).to include("Goalが入力されていません。")
      end
      ...
      it 'passwordが5文字以下であれば登録できない' do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include("Passwordは6文字以上に設定して下さい。", "Passwordは有効でありません。")
      end
      ...
      it 'passwordが数字のみの場合は登録できない' do
        ...
        @user.valid?
        expect(@user.errors.full_messages).to include("Passwordは有効でありません。")
      end
      it 'passwordがローマ字のみの場合は登録できない' do
        @user.password = 'abcdef'
        @user.password_confirmation = 'abedef'
        @user.valid?
        expect(@user.errors.full_messages).to include("Password confirmationが内容とあっていません。", "Passwordは有効でありません。")
      end
      ...
      it 'passwordが全角の場合は登録できない' do
        ...
        @user.password_confirmation = 'abedef'
        @user.valid?
        expect(@user.errors.full_messages).to include("Password confirmationが内容とあっていません。", "Passwordは6文字以上に設定して下さい。", "Passwordは有効でありません。")
      end
